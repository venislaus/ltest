...
<?php
$sql = "SELECT * FROM ".$table_events." where events_featured = '1' order by ordering ASC LIMIT 1";
...
if(!empty($event_items)){
    ...
    ?>
        <div class="featuredEvent">
            <img src="<?php echo plugins_url() ."/events/images/events/thumbsize1/".$item['id']."/".$item['events_image']; ?>">
            <span class="eventDate"><?php echo date('M D',strtotime($item['events_date']));?></span>
            <span class="eventName"><?=$item['events_name']?></span>
            <span class="eventDescription"><?=substr($item['events_description'],0,100);?></span>
            <a href="<?=$item['events_link']?>" target="_blank" class="eventLink">Button</a>
        </div>
    <?php
    }
}
...
<?php
$sql = "SELECT * FROM ".$table_event_category." order by ordering ASC";
