...
<?php $items = $conversation_role_items; ?>
<div class="wrap">
    <h1>Role
...
<form name="role_list" id="role_list" class="admin_style" action="" method="POST">
<?php if(!empty($items)){?>
    <div class="actions">
        <select name="action">
        ...
        <input type="text" name="s_role_name" value="<?php echo (isset($_REQUEST['s_role_name']))? $_REQUEST['s_role_name'] : ''; ?>" placeholder="Role Name" />
        <input class="button" type="submit" value="Search" name="search_role_list">
        <input class="button" type="button" name="btnreset" value="Reset" onclick="window.location.href='?page=conversation_role'" />
    </div>
    <div>
        <input class="button" type="submit" value="Update Order" name="update_role_ordering">
    </div>
<?php } ?>
<table id="all_role_lists" cellspacing="0" width="100%" class="wp-list-table widefat">
	<thead>
		<tr>
			...
			<th style="text-align:left" class="manage-column column-username sortable desc" id="username" scope="col">
                <span>
                    <?php
                     $n_sortorder = "asc";
                     if($_REQUEST['sortby'] == "role_name" && $_REQUEST['sortorder'] == "asc"){
                         $n_sortorder = "desc";
                     }
                    ?>
                <a href="?page=conversation_role&s_role_name=<?php echo $_REQUEST['s_role_name']?>&paging=<?php echo $_REQUEST['paging']?>&sortby=role_name&sortorder=<?=$n_sortorder?>">Role Name</a>
                </span>
            </th>
    ...
    <?php
		...
		?>
			<tr >	
                ...
                <td style="text-align:center" class="input_name">
				    <a href="?page=conversation_role&action=edit&role_id=<?php echo $item['id']; ?>&s_role_name=<?php echo $_REQUEST['s_role_name']?>&paging=<?php echo $_REQUEST['paging']?>&sortby=<?php echo $_REQUEST['sortby']; ?>&sortorder=<?php echo $_REQUEST['sortorder']; ?>"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>
		...
		<?php }else{ ?>
			<tr>
				 <td colspan="8" class="check" scope="row" style="margin: 4px;">No role available</td>
			</tr>
		<?php }
	?>       
