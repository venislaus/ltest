...
<?php $items = $conversation_territory_items; ?>
<div class="wrap">
    <h1>Territory
...
<form name="territory_list" id="territory_list" class="admin_style" action="" method="POST">
<?php if(!empty($items)){?>
    <div class="actions">
        <select name="action">
        ...
        <input type="text" name="s_territory_name" value="<?php echo (isset($_REQUEST['s_territory_name']))? $_REQUEST['s_territory_name'] : ''; ?>" placeholder="Territory Name" />
        <input class="button" type="submit" value="Search" name="search_territory_list">
        <input class="button" type="button" name="btnreset" value="Reset" onclick="window.location.href='?page=conversation_territory'" />
    </div>
    <div>
        <input class="button" type="submit" value="Update Order" name="update_territory_ordering">
    </div>
<?php } ?>
<table id="all_territory_lists" cellspacing="0" width="100%" class="wp-list-table widefat">
	<thead>
		<tr>
			...
			<th style="text-align:left" class="manage-column column-username sortable desc" id="username" scope="col">
                <span>
                    <?php
                     $n_sortorder = "asc";
                     if($_REQUEST['sortby'] == "territory_name" && $_REQUEST['sortorder'] == "asc"){
                         $n_sortorder = "desc";
                     }
                    ?>
                <a href="?page=conversation_territory&s_territory_name=<?php echo $_REQUEST['s_territory_name']?>&paging=<?php echo $_REQUEST['paging']?>&sortby=territory_name&sortorder=<?=$n_sortorder?>">Territory Name</a>
                </span>
            </th>
    ...
    <?php
		...
		?>
			<tr >	
                ...
                <td style="text-align:center" class="input_name">
				    <a href="?page=conversation_territory&action=edit&territory_id=<?php echo $item['id']; ?>&s_territory_name=<?php echo $_REQUEST['s_territory_name']?>&paging=<?php echo $_REQUEST['paging']?>&sortby=<?php echo $_REQUEST['sortby']; ?>&sortorder=<?php echo $_REQUEST['sortorder']; ?>"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>
		...
		<?php }else{ ?>
			<tr>
				 <td colspan="8" class="check" scope="row" style="margin: 4px;">No territory available</td>
			</tr>
		<?php }
	?>       
...
<script>
    ...
    jQuery(function($){
        $("#all_territory_lists tbody").sortable({
            appendTo: "parent",
            helper: "clone"
        });
        ...
        $('#territory_list').submit(function(){
